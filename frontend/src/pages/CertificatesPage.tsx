...
function sourceLabel(source: string): string {
  return source === "imported" ? "Imported" : "Let's Encrypt";
}
...
export function CertificatesPage() {
  ...
  const [confirm, setConfirm] = useState<
    { action: "renew" | "dry-run" | "delete"; name?: string } | null
  >(null);
  ...
  const importCertificate = async (event: FormEvent) => {
    event.preventDefault();
    if (!certificateFile || !privateKeyFile) {
      showError("Certificate and private key files are required");
      return;
    }
    try {
      const result = await api.importCertificate({
      ...
      showSuccess(result.message);
      queryClient.invalidateQueries({ queryKey: ["certificates"] });
      queryClient.invalidateQueries({ queryKey: ["certificate-renewal-history"] });
      setImportName("");
      setImportDomain("");
      ...
      showError(error instanceof ApiError ? error.message : "Import failed");
    }
  };
  ...
  const runConfirm = async () => {
    ...
    try {
      if (confirm.action === "renew" && confirm.name) {
        ...
        showSuccess(result.message);
      } else if (confirm.action === "delete" && confirm.name) {
        const result = await api.deleteCertificate(confirm.name);
        showSuccess(result.message);
      } else {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card title="Import purchased certificate">
        <p className="mb-3 text-sm text-white/60">
          Upload PEM files for certificates you bought from a commercial CA or another provider. The primary domain must
          match a name on the certificate. Re-import before expiry to replace an imported certificate.
        </p>
        <form className="flex flex-col gap-3" onSubmit={importCertificate}>
          <div className="grid gap-3 md:grid-cols-2">
            <input
              value={importName}
              onChange={(e) => setImportName(e.target.value)}
              placeholder="Certificate name (e.g. shop.example.com)"
              required
              className="w-full"
            />
            <input
              ...
              placeholder="Primary domain (e.g. shop.example.com)"
              required
              className="w-full"
            />
          </div>
      ...
      <Card title="Issue Let's Encrypt certificate">
        {!emailConfigured ? (
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {data.map((cert) => {
                  ...
                  return (
                  <tr key={`${cert.source}-${cert.name}`} className="border-b border-white/5">
                    <td className="px-3 py-3">{cert.name}</td>
                    <td className="px-3 py-3">{cert.domains.join(", ")}</td>
                    <td className="px-3 py-3">{sourceLabel(cert.source)}</td>
                    ...
                    <td className="px-3 py-3">
                      <div className="flex flex-wrap gap-2">
                        {cert.renewable ? (
                          ...
                          <button
                            className="rounded bg-red-500/20 px-2 py-1 text-red-100"
                            onClick={() => setConfirm({ action: "delete", name: cert.name })}
                          >
                            Delete
                          </button>
                        )}
              ...
              <tbody>
                {renewalHistory.map((entry: CertificateRenewalLogEntry) => (
                  <tr key={entry.id} className="border-b border-white/5">
                    ...
                    <td className="px-3 py-3 capitalize">{entry.action.replace("_", " ")}</td>
                    <td className="px-3 py-3">
        ...
        message={
          confirm?.action === "renew"
            ...
            : confirm?.action === "delete"
              ? `Delete imported certificate ${confirm.name}? Proxies using it must be updated before removal.`
